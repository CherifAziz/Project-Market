...
signal close_requested(company_id: String)
...
@onready var realized_label: Label = %RealizedPnL
...
func _ready() -> void:
	...
	close_button.pressed.connect(func() -> void: close_requested.emit(company_id))
...
func refresh() -> void:
	...
	realized_label.text = "REALIZED THIS RUN   %s" % MoneyFormat.pnl(_market.get_realized_pnl(company_id))
	close_button.visible = _market.has_open_position(company_id)
	close_button.disabled = not _market.is_trading_enabled()
	close_button.text = "CLOSE %s  //  REALIZE P&L" % company.ticker
	...
	if _market.has_open_position(company_id):
		...
		position_pnl.text = "UNREALIZED P&L   %s" % MoneyFormat.pnl(pnl)
		...
		position_pnl.text = ""
		short_button.disabled = not _market.is_trading_enabled()
